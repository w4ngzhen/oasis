...
impl TileRenderDescriptor {
    fn new(index: usize, color: Color, bg_color: Color) -> Self {
        Self { index, color, bg_color: Some(bg_color) }
    }

    fn new_with_no_bg_color(index: usize, color: Color) -> Self {
        Self { index, color, bg_color: None }
    }
}
...
pub fn tile_to_render_descriptor(tile_type: TileType) -> Option<TileRenderDescriptor> {
    ...
    match tile_type {
        ...
        TileType::Floor => Some(TileRenderDescriptor::new_with_no_bg_color(219, floor_color)),
